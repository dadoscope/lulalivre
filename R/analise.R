...
p2 <- df %>% filter(lang != "pt") %>% group_by(screen_name) %>% mutate(total = sum(retweet_count)) %>% ungroup() %>% group_by(screen_name, lang, total) %>% summarise(parcial = sum(retweet_count)) %>% arrange(total) %>% tail(20) %>% ggplot(aes(x = reorder(screen_name, total), y = parcial, fill = lang)) + geom_bar(stat = "identity") + theme_bw()+coord_flip()+labs(title = "Número de retuítes contendo a palavra 'Lula'", y = "Número de Retuítes", x = "Perfis do Twitter")

p3 <- df %>% filter(lang == "pt") %>% group_by(screen_name) %>% mutate(total = sum(retweet_count)) %>% ungroup() %>% group_by(screen_name, lang, total) %>% summarise(parcial = sum(retweet_count)) %>% arrange(total) %>% tail(20) %>% ggplot(aes(x = reorder(screen_name, total), y = parcial, fill = lang)) + geom_bar(stat = "identity") + theme_bw()+coord_flip()+labs(title = "Número de retuítes contendo a palavra 'Lula' - Português", y = "Número de Retuítes", x = "Perfis do Twitter")

p4 <- df %>% filter(lang == "es") %>% group_by(screen_name) %>% mutate(total = sum(retweet_count)) %>% ungroup() %>% group_by(screen_name, lang, total) %>% summarise(parcial = sum(retweet_count)) %>% arrange(total) %>% tail(20) %>% ggplot(aes(x = reorder(screen_name, total), y = parcial, fill = lang)) + geom_bar(stat = "identity") + theme_bw()+coord_flip()+labs(title = "Número de retuítes contendo a palavra 'Lula' - Espanhol", y = "Número de Retuítes", x = "Perfis do Twitter")

p5 <- df %>% filter(lang == "en") %>% group_by(screen_name) %>% mutate(total = sum(retweet_count)) %>% ungroup() %>% group_by(screen_name, lang, total) %>% summarise(parcial = sum(retweet_count)) %>% arrange(total) %>% tail(20) %>% ggplot(aes(x = reorder(screen_name, total), y = parcial, fill = lang)) + geom_bar(stat = "identity") + theme_bw()+coord_flip()+labs(title = "Número de retuítes contendo a palavra 'Lula' - Inglês", y = "Número de Retuítes", x = "Perfis do Twitter")
